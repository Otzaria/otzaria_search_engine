...
- `maxExpansions` (int/u32): Regex-expansion ceiling. A single term truncates its term collection at the ceiling (partial results, flagged via the status-bearing variants). A multi-term phrase uses it as an engine switch: at or under the ceiling the exact tantivy `RegexPhraseQuery` runs; over it the engine falls back to a term-list phrase built from per-position materialized term sets (per-position caps, truncation flagged) — never an error
