...
use search_engine::api::benchmark::{RegexBenchmarker, TestCase, BenchmarkResult};
...
fn run_custom_benchmark(benchmarker: &mut RegexBenchmarker) -> Result<()> {
    ...
    let custom_queries = vec![
        ...
        TestCase {
            ...
            regex_terms: vec!["משנה".to_string(), "[א-ת]{1,3}".to_string(), "פרק".to_string(), "[א-ט]".to_string()],
            facets: vec![],
            slop: 4,
            max_expansions: 100,
        },
        ...
        TestCase {
            ...
            regex_terms: vec!["שו\"ת".to_string(), "[א-ת]{3,10}".to_string(), "סימן".to_string()],
            facets: vec![],
            slop: 3,
            max_expansions: 200,
        },
        TestCase {
            ...
            regex_terms: vec!["(ספירות|פרצופים|עולמות)".to_string(), "[א-ת]{2,6}".to_string()],
            facets: vec![],
            slop: 2,
            max_expansions: 150,
        },
        TestCase {
            ...
            regex_terms: vec!["[כ-מ][ת-ת][ב-ו]".to_string(), "[א-ה].*".to_string(), "[ל-ן]{1,2}".to_string()],
            facets: vec![],
            slop: 3,
            max_expansions: 400,
        },
...
fn analyze_performance_patterns(results: &[BenchmarkResult]) {
    ...
    let single_term: Vec<_> = results.iter().filter(|r| r.regex_terms.len() == 1).collect();
    let multi_term: Vec<_> = results.iter().filter(|r| r.regex_terms.len() > 1).collect();
    
    if !single_term.is_empty() {
        let avg_single = single_term.iter().map(|r| r.execution_time_ms).sum::<u64>() as f64 / single_term.len() as f64;
        println!("Single-term queries average: {:.2}ms", avg_single);
    }
    
    if !multi_term.is_empty() {
        let avg_multi = multi_term.iter().map(|r| r.execution_time_ms).sum::<u64>() as f64 / multi_term.len() as f64;
    ...
    if !no_slop.is_empty() && !with_slop.is_empty() {
        let avg_no_slop = no_slop.iter().map(|r| r.execution_time_ms).sum::<u64>() as f64 / no_slop.len() as f64;
        let avg_with_slop = with_slop.iter().map(|r| r.execution_time_ms).sum::<u64>() as f64 / with_slop.len() as f64;
        
        println!("No slop average: {:.2}ms", avg_no_slop);
        println!("With slop average: {:.2}ms", avg_with_slop);
        println!("Slop performance impact: {:.1}x slower", avg_with_slop / avg_no_slop);
    }
    ...
    if !low_expansion.is_empty() && !high_expansion.is_empty() {
        let avg_low = low_expansion.iter().map(|r| r.execution_time_ms).sum::<u64>() as f64 / low_expansion.len() as f64;
        let avg_high = high_expansion.iter().map(|r| r.execution_time_ms).sum::<u64>() as f64 / high_expansion.len() as f64;
    ...
    let slow_queries: Vec<_> = results.iter().filter(|r| r.execution_time_ms > 200).collect();
    if !slow_queries.is_empty() {
        println!("\nPerformance bottleneck patterns:");
        for query in slow_queries {
            let pattern_analysis = analyze_regex_complexity(&query.regex_terms);
            println!("  - {}: {} ({})", query.query_name, pattern_analysis, query.execution_time_ms);
        }
    }
}
...
fn analyze_hebrew_patterns(results: &[BenchmarkResult]) {
    ...
    if !talmudic.is_empty() {
        let avg = talmudic.iter().map(|r| r.execution_time_ms).sum::<u64>() as f64 / talmudic.len() as f64;
        println!("  Talmudic patterns average: {:.2}ms", avg);
    }
    
    if !biblical.is_empty() {
        let avg = biblical.iter().map(|r| r.execution_time_ms).sum::<u64>() as f64 / biblical.len() as f64;
        println!("  Biblical patterns average: {:.2}ms", avg);
    }
    
    if !halachic.is_empty() {
        let avg = halachic.iter().map(|r| r.execution_time_ms).sum::<u64>() as f64 / halachic.len() as f64;
        println!("  Halachic patterns average: {:.2}ms", avg);
    }
    
    if !other.is_empty() {
        let avg = other.iter().map(|r| r.execution_time_ms).sum::<u64>() as f64 / other.len() as f64;
