...
pub fn run_focused_benchmark() -> Result<()> {
    ...
    let test_cases = vec![
        ...
        // Two words exact
        ("Two words exact", vec!["בית".to_string(), "המקדש".to_string()], 0, 50),
        
        // Two words with wildcards
        ("Two words with wildcards", vec![".*בית.*".to_string(), ".*המקדש.*".to_string()], 0, 5000),
        
        // Complex pattern
        ("Complex Hebrew pattern", vec!["[א-ת]{2,4}".to_string()], 0, 1000),
        ...
        ("Three words with wildcards", vec![".*רבי.*".to_string(), ".*יהודה.*".to_string(), ".*הנשיא.*".to_string()], 0, 8000),
        
        // Alternation pattern
        ("Alternation pattern", vec!["(משה|אהרן|מרים)".to_string()], 0, 1000),
        
        // Complex Hebrew root pattern
        ("Hebrew root pattern", vec!["[כ-ל][ת-ת][ב-ב]".to_string()], 0, 2000),
    ];
