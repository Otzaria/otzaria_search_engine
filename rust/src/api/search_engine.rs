...
use tantivy::query::{RegexPhraseQuery, Query};
use tantivy::{
    doc, DocAddress, IndexReader, IndexWriter, Order, ReloadPolicy, Score, Searcher,
};
...
impl SearchEngine {
    ...
    pub fn new(path: &str) -> Self {
        ...
        SearchEngine { schema, index, index_writer, index_reader }
    }
    ...
    pub fn upsert_document(
        ...
        self.add_document(_id, _title, _reference, _topics, _text, _segment, _is_pdf, _file_path)
    }
    ...
    pub fn remove_documents_by_title(&mut self, title: &str) -> Result<()> {
        ...
        self.index_writer.delete_term(Term::from_field_text(title_field, title));
        Ok(())
    }
    ...
    pub fn count(
        ...
        let query = Self::build_query(&self.index, regex_terms, facets.clone(), slop, max_expansions)?;
        let searcher = self.index_reader.searcher();
    ...
    pub fn get_facet_counts(
        ...
        let results = facet_counts
            ...
            .map(|(f, count)| FacetCount { path: f.to_string(), count })
            .collect();
        Ok(results)
    }
    ...
    fn build_results(
        ...
        for doc_address in addresses {
            ...
            let result_text = if snippet_html.is_empty() { text } else { snippet_html };

            results.push(SearchResult { title, reference, text: result_text, id, segment, is_pdf, file_path });
        }
        Ok(results)
    }
}
...
mod tests {
    ...
    fn test_delete_document_by_id() {
        ...
        assert_eq!(engine.count(vec!["שלום".to_string()], &vec!["/root".to_string()], 0, 100).unwrap(), 2);

        engine.delete_document_by_id(1).unwrap();
        engine.commit().unwrap();

        assert_eq!(engine.count(vec!["שלום".to_string()], &vec!["/root".to_string()], 0, 100).unwrap(), 1);
    }
    ...
    fn test_upsert_document() {
        ...
        assert_eq!(engine.count(vec!["טקסט".to_string()], &vec!["/root".to_string()], 0, 100).unwrap(), 1);
        assert_eq!(engine.count(vec!["ישן".to_string()], &vec!["/root".to_string()], 0, 100).unwrap(), 0);
        assert_eq!(engine.count(vec!["חדש".to_string()], &vec!["/root".to_string()], 0, 100).unwrap(), 1);
    }
    ...
    fn test_search_fuzzy() {
        ...
        let exact = engine
            .search_fuzzy(vec!["שלום".to_string()], vec!["/root".to_string()], 10, 0, 0, ResultsOrder::Relevance, None)
            .unwrap();
        let exact_texts: Vec<&str> = exact.iter().map(|r| r.text.as_str()).collect();
        assert!(exact_texts.contains(&"שלום"), "distance=0 must return exact match");
        assert!(!exact_texts.contains(&"שלם"), "distance=0 must not return near-match");

        // distance=1: must return both "שלום" and the near-match "שלם"
        let fuzzy = engine
            .search_fuzzy(vec!["שלום".to_string()], vec!["/root".to_string()], 10, 0, 1, ResultsOrder::Relevance, None)
            .unwrap();
        let fuzzy_texts: Vec<&str> = fuzzy.iter().map(|r| r.text.as_str()).collect();
        assert!(fuzzy_texts.contains(&"שלום"), "distance=1 must return exact match");
        assert!(fuzzy_texts.contains(&"שלם"), "distance=1 must return near-match one edit away");
        assert!(!fuzzy_texts.contains(&"ביי"), "unrelated term must not appear");
    }
    ...
    fn test_search_and_count() {
        ...
        let page = engine
            .search_and_count(vec!["שלום".to_string()], vec!["/root".to_string()], 1, 0, 0, 100, ResultsOrder::Relevance, None)
            .unwrap();

        assert_eq!(page.total_count, 2, "total_count should reflect all hits, not just page size");
        assert_eq!(page.results.len(), 1, "results should be limited by limit param");
    }
    ...
    fn test_search_offset() {
        ...
        let page1 = engine
            .search(vec!["שלום".to_string()], vec!["/root".to_string()], 2, 0, 0, 100, ResultsOrder::Catalogue, None)
            .unwrap();
        let page2 = engine
            .search(vec!["שלום".to_string()], vec!["/root".to_string()], 2, 2, 0, 100, ResultsOrder::Catalogue, None)
            .unwrap();
    ...
    #[test]
    fn test_optimize_reduces_segments() {
        let (mut engine, _dir) = make_engine();
        // Each commit creates a segment
        add(&mut engine, 1, "שלום", "/books/a.txt");
        engine.commit().unwrap();
        add(&mut engine, 2, "עולם", "/books/b.txt");
        engine.commit().unwrap();
        ...
        assert_eq!(after, 1, "after optimize there should be exactly one segment");
        // Reader must be reloaded: count should still work
        assert_eq!(engine.get_document_count(), 2);
    }
}
