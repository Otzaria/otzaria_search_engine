...
// OTZARIA LOCAL PATCH (the only change in this vendored copy — see
// vendor/tantivy-fst/README.md): raised from the upstream 1_000. Hebrew
// morphological phrase patterns join many wildcard-wrapped branches whose
// overlap inflates the determinized state count far past 1_000 while each
// branch alone is tiny. A state costs ~4KB+ (`next: [Option<usize>; 256]`),
// so the worst-case DFA is ~32MB+, allocated transiently during query
// compilation and freed immediately after.
const STATE_LIMIT: usize = 8_192;
