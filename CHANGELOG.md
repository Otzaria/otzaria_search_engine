...
- **מאמת המרווחים כבר לא סורק מחדש את מילון הטרמים.** `GapVerifiedWeight`
  ביצע סריקת FST מלאה של תבנית כל מילה פר-סגמנט **בכל קריאת מנוע** (חיפוש +
  ספירה + ספירה-לפי-ספר + פאסטים = ‏4–5 סריקות כפולות לחיפוש משתמש אחד).
  כעת שני מנועי הפרזה (המדויק וה-degrade) קוראים את ה-postings הפוזיציוניים
  מקבוצות המונחים שכבר מומשו ונשמרו במטמון בבניית השאילתה — lookups נקודתיים
  (`get_term_info`) במקום סריקה, דרך helper משותף (`positional_postings`).

- **מימוש עמדות הפרזה ומילות הטווחים ממוקבל.** סריקות ה-FST פר-מילה הן
  החלק היקר של בניית שאילתה רחבה והן בלתי-תלויות זו בזו — כעת הן רצות
  במקביל על ה-pool של rayon (גם ב-`phrase_query_with_degrade`, עם איחוד
  מילים כפולות לסריקה אחת, וגם ב-`scoped_words_query` של טווחי
  פסקה/כותרת). שאילתת 4 מילים עם אפשרויות רחבות נבנית עד פי-4 מהר יותר
  בקריאה הראשונה (הבאות מוגשות מהמטמון ממילא).
...
- **שאילתות פרזה רחבות כבר לא נכשלות על תקרת ההרחבות — degrade במקום שגיאה.**
  ...
  התיקון (`phrase_query_with_degrade`): כל עמדת-מילה ממומשת תחילה לקבוצת
  מונחים תחת תקציבי איסוף **פר-עמדה** (`PHRASE_POSITION_MAX_EXPANSIONS =
  8,192` מונחים, תקציב postings של 20M — אותם סריקות FST ממוטמנות של מסלול
  המילה הבודדת). הספירה בוחרת מנוע ביצוע:

  - **בתוך התקרה המצטברת** — `RegexPhraseQuery` ההיסטורי (התנהגות וניקוד
