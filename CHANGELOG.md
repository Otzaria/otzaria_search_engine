...
- **טוקן-תאום נטול-גרשיים באינדוקס** – במצב האינדוקס (`emit_quote_free`),
  ...
  שינוי סכימת tantivy — **מחייב העלאת `INDEX_SCHEMA_VERSION` לפני
  פרסום** כדי שאינדקסים ישנים ייבנו מחדש (בדיקת התאימות לא תופסת זאת
  לבד; טרם הועלה — ראו ההערה ליד הקבוע).
...
- **`SearchStreamUpdate.truncated` – איתות "תוצאות חלקיות" ל-UI** – כשמסלול
  ...
  החיפוש". המסלולים המדויק והמקורב אינם מתדרדרים כך ולכן תמיד `false`.
