...
- **`semantic_corpus::TantivyCorpus` — מימוש `CorpusIndex` ו-`CorpusBooks` מעל
  ...
  **`corpus_id`** נגזר מאותו snapshot: SHA-256 מעל כל מסמך חי בסדר `line_id`
  עולה, עם המזהה, מפתח הספר והטקסט, כל אחד באורך מקודם. כותרות, מראי מקום
  ו-facets מחוץ לו במכוון — הם מושווים רשומה-רשומה באימות הארטיפקט, ובזמן
  שאילתה האפליקציה טוענת אותם מ-Tantivy ולא מהארטיפקט, ולכן ספר ששונתה כותרתו
  אינו סיבה לבנות שישה מיליון וקטורים מחדש.
