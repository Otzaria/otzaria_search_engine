...
## 0.7.4 – 2026-08-23 – חסימת flutter_rust_bridge מתחת ל-2.13.0 + תגי שבירה כרווח

### Fixed

- **אילוץ ה-Dart של `flutter_rust_bridge` הוצר ל-`>=2.12.0 <2.13.0`.** צד
  ה-Rust מוצמד `=2.12.0` והקוד המחולל הוא codegen 2.12.0, אבל האילוץ הפתוח
  `^2.12.0` אפשר ל-pub לפתור את ה-runtime של 2.13.0 (פורסם 23/8) — ואתחול
  `RustLib.init` נכשל על אי-התאמת גרסאות. כל צרכן שעשה `pub get` טרי קיבל
  מנוע שאינו עולה.
