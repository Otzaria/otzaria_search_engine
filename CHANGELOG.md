...
- **תקרת הקיבוץ שומרת את הקבוצות הטובות ביותר** – בהגעה ל-50,000 קבוצות
  ה-collector השמיט כל קבוצה *חדשה* לפי סדר סריקת המסמכים והסגמנטים —
  קבוצה טובה (למשל id נמוך במיון קטלוגי, או ציון גבוה ברלוונטיות) שהגיעה
  מאוחר נזרקה בעוד גרועות ממנה נשמרו, והעמוד הראשון היה שגוי. כעת בתקרה
  הקבוצה *הגרועה* לפי סדר המיון מפנה את מקומה לקבוצה טובה ממנה (אינדקס
  BTreeSet לצד המפה, גם פר-סגמנט וגם במיזוג) — כל עמוד בטווח התקרה מדויק.
  שארית ה-degrade תחת `truncated`: ‏`group_count` נשאר תחתית, ומונה של
  קבוצה שפונתה וחזרה מאבד את חבריה המוקדמים.
