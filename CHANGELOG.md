...
- **`semantic_corpus::TantivyCorpus` — מימוש `CorpusIndex` ו-`CorpusBooks` מעל
  ...
  **קריאה קפדנית, בלי ברירות מחדל.** טקסט חסר אינו `""`, עמודה חסרה אינה `0`
  ...
  **סכמת ה-IDs נאכפת ולא רק מוצהרת.** `add_document` הציבורי מקבל כל `u64`,
  ...
  חצי עליון אחד, אין חצי עליון משותף לשני ספרים, ואין שורה במיקום 0. רציפות
  **אינה** נדרשת — מחיקת שורה משאירה חור, וקורפוס עם שורה שנמחקה הוא קורפוס רגיל.

- **`build_semantic_artifact` — בינארי למכונת ה-build.** נתיב אינדקס, קובץ מודל
  ומתכון → ארטיפקט מאומת. עד עכשיו המסלול המלא רץ רק בתוך בדיקה; זה הופך אותו
